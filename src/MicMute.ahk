...
#Include, %A_ScriptDir%\UI\voicemeter
#Include, VoiceMeter.ahk
...
; Wait for Explorer shell and system tray to be ready (important for startup task)
if(!arg_reload){
    util_log("[Main] Waiting for Explorer shell...")
    WinWait, ahk_class Shell_TrayWnd, , 30  ; Wait up to 30 seconds for system tray
    if(ErrorLevel){
        util_log("[Main] Warning: System tray not found within 30 seconds")
    }else{
        util_log("[Main] Explorer shell ready")
        Sleep, 500  ; Small delay to ensure tray is fully initialized
    }
}
