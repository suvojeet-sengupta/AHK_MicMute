...
tray_init(){
    ...
    util_log("[Tray] Initializing tray menu")
    tray_defaults()

    ; Simple menu structure
    tray_add("Toggle microphone", Func("tray_noFunc"))
    Menu, Tray, Add, ; separator line
    tray_add("Voice Meter", Func("voicemeter_toggle"))
...
tray_defaults(){
    ...
    Loop, 3 {  ; Retry up to 3 times
        Try {
            ...
            util_log("[Tray] Failed to set tray icon, attempt " A_Index)
            Sleep, 500
        }
    }
    util_log("[Tray] Failed to set tray icon after 3 attempts")
}
...
tray_update(mic_obj){
    ...
    Loop, 3 {  ; Retry up to 3 times
        Try {
            ...
            util_log("[Tray] Failed to update tray icon, attempt " A_Index)
            Sleep, 300
        }
    }
}
